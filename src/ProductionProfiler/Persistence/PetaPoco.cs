...
namespace ProductionProfiler.Core.Persistence
{
    ...
    public class Database : IDisposable
    {
        ...
        public string UpdateSql(string tableName, string primaryKeyName, object poco, object primaryKeyValue)
        {
            var pd = PocoData.ForObject(poco, primaryKeyName);
            var index = 0; 
            var sb = new StringBuilder();
            ...
            return string.Format("UPDATE {0} SET {1} WHERE {2} = {3}{4}", EscapeTableName(tableName), sb, EscapeSqlIdentifier(primaryKeyName), _paramPrefix, index++);
        }
